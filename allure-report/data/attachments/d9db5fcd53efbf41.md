...
```ts
  ...
  5  |     this.page = page;
  6  | 
  7  |     this.cart = page.locator('.shopping_cart_link');
  8  |     this.cartBadge = page.locator('.shopping_cart_badge');
  9  |   }
  10 | 
  11 |   async addProduct(product) {
  12 |     await this.page
  13 |       .locator(`[data-test="add-to-cart-${product}"]`)
  14 |       .click();
  15 | 
  16 |     await this.attachScreenshot('add one product');
  17 |   }
  18 | 
  19 |   async addMultipleProducts(products) {
  20 |     for (const product of products) {
  21 |       await this.addProduct(product);
  22 |     }
  23 |   }
  24 | 
  25 |   async getCartCount() {
  26 |     return await this.cartBadge.textContent();
  27 |   }
  28 | 
  29 |   async openCart() {
> 30 |     await this.cart.click();
     |                     ^ Error: locator.click: Test timeout of 30000ms exceeded.
  31 |     await this.attachScreenshot('open cart');
  32 |   }
  33 | 
  34 |   async attachScreenshot(name) {
  35 |     await test.info().attach(name, {
  36 |       body: await this.page.screenshot(),
