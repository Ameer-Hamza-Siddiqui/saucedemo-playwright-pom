...
- Name: tests\api.spec.js >> Validate Book API Response
- Location: tests\api.spec.js:11:5
...
```ts
  ...
  12 | 
  13 | const response = await request.get('https://api-testing-postman.vercel.app/');
  14 | 
> 15 | const responseBody = await response.json();
  ...
  27 | 'https://demoqa.com/Account/v1/Authorized',
  28 | {
  29 |   data: {
  ...
  70 | //npx playwright test tests/Api.spec.js -g "Create User API Test_2"
  71 | //npx playwright test tests/Api.spec.js -g "Create User API Test_3"
```