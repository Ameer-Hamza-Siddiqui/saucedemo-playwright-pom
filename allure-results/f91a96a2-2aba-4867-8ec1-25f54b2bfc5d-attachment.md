...
Call log:
  - waiting for locator('#logout_sidebar_link') to be visible
    47 × locator resolved to hidden <a href="#" id="logout_sidebar_link" class="bm-item menu-item" data-test="logout-sidebar-link">Logout</a>
...
```ts
  ...
  16 | 
  17 |   async logout() {
  18 |     await this.menuButton.waitFor({ state: 'visible' });
  19 | 
  20 |     // ⭐ CHANGE 1: menu button click add kiya
  21 |     await this.menuButton.click();
  22 | 
  23 |     await this.attachScreenshot('05 After clicking menu button');
  24 | 
  25 |     // ⭐ CHANGE 2: logout button ke liye wait add kiya
> 26 |     await this.logoutButton.waitFor({ state: 'visible' });
     |                             ^ Error: locator.waitFor: Test timeout of 30000ms exceeded.
  27 | 
  28 |     await this.logoutButton.click();
  29 | 
  30 |     await this.attachScreenshot('06 After clicking logout');
  31 |   }
  32 | 
  33 | }
  34 | 
  35 | export default Logout;
```